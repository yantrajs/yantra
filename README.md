...
## Internal Usage Single Server License
For any application, if you are executing JavaScript automatically such as server side rendering, where your end users are not authoring any JavaScript code, you can purchase Runtime License. You can use other tools to automatically generate the code, but your users cannot. You can also use ExpressionCompiler to dynamically generate IL, save and execute it. This license is limited to a single website or single application.
### Restrictions
* Application should not execute code generated by User
* Users cannot inspect the code

## Application Runtime License
This license grants you permission to execute any JavaScript code or use ExpressionCompiler to generate and execute any code as long as it is not sublicensed to compete with `YantraJS` and it is not used as network service for other applications to execute scripts. This license is limited to single web application domain or single application. If you are making two different applications for two different kind of users, you should purchase two application runtime licenses. If you are making two web sites for two different kind of client's you should purchase two different application runtime license.
