...
public static new JSFunction CreateClass(JSContext context, bool register = true) {
                        ...
                        var @class = new JSClassFunction(context, (in Arguments a) => new JSUInt32Array(in a)
                            ...
                            , length:3);
                        if (register) {
                            context[KeyString.Uint32Array] = @class;
                        }
...
@class.FastAddValue(KeyString.from, new JSFunction(context, JSUInt32Array.From, "from" ,"function from() { [native] }", createPrototype: false, length: 1), JSPropertyAttributes.ConfigurableValue);
// Exporting Of as of
@class.FastAddValue(KeyString.of, new JSFunction(context, JSUInt32Array.Of, "of" ,"function of() { [native] }", createPrototype: false), JSPropertyAttributes.ConfigurableValue);
context.Uint32Array_Prototype = prototype.PrototypeObject;
return @class;
}
}
}
