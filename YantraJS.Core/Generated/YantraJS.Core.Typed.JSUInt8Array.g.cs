...
public static new JSFunction CreateClass(JSContext context, bool register = true) {
                        ...
                        var @class = new JSClassFunction(context, (in Arguments a) => new JSUInt8Array(in a)
                            ...
                            , length:3);
                        if (register) {
                            context["Uint8Array".ToKeyString()] = @class;
                        }
...
@class.FastAddValue("from".ToKeyString(), new JSFunction(context, JSUInt8Array.From, "from" ,"function from() { [native] }", createPrototype: false, length: 1), JSPropertyAttributes.ConfigurableValue);
// Exporting Of as of
@class.FastAddValue("of".ToKeyString(), new JSFunction(context, JSUInt8Array.Of, "of" ,"function of() { [native] }", createPrototype: false), JSPropertyAttributes.ConfigurableValue);
return @class;
}
}
}
