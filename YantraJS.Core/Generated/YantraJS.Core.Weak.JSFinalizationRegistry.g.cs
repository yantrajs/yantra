...
public static JSFunction CreateClass(JSContext context, bool register = true) {
                        ...
                        if (register) {
                            context[KeyString.FinalizationRegistry] = @class;
                        }
                        prototype = @class.prototype;
                        
// Exporting Unregister as unregister
prototype.FastAddValue(KeyString.unregister, new JSFunction(context, (in Arguments a) =>
...
prototype.FastAddValue(KeyString.register, new JSFunction(context, (in Arguments a) =>
                    a.This is JSFinalizationRegistry @this
                        ? @this.Register(in a)
                        : throw JSContext.Current.NewTypeError("Failed to convert this to JSFinalizationRegistry")
                        , "register"
                        ,"function register() { [native] }", createPrototype: false), JSPropertyAttributes.ConfigurableValue);
context.FinalizationRegistry_Prototype = prototype.PrototypeObject;
return @class;
}
}
}
