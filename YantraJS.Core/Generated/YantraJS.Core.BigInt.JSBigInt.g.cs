...
public static new JSFunction CreateClass(JSContext context, bool register = true) {
                        ...
                        if (register) {
                            context[KeyString.BigInt] = @class;
                        }
...
@class.FastAddValue(KeyString.asIntN, new JSFunction(context, (in Arguments a) => {
...
@class.FastAddValue(KeyString.asUintN, new JSFunction(context, (in Arguments a) => {
			var pbits = JSValueToClrConverter.ToLong(a[0], "bits");
			var pbigint = JSValueToClrConverter.GetAsOrThrow<YantraJS.Core.BigInt.JSBigInt>(a[1], "bigint");
...
context.BigInt_Prototype = prototype.PrototypeObject;
return @class;
}
}
}
