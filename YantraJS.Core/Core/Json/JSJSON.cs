...
public partial class JSJSON : JSObject
{
    ...
    private static void QuoteString(in StringOrChar input, TextWriter result)
    {
        if (input.IsChar)
        {
            result.Write('"');
        ...
        QuoteString(input.ToString(), result);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void WriteCharacter(char c, TextWriter result)
    {
        switch (c)
        {
            case '\"':
